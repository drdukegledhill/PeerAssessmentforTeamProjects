...
Note: The web UI displays a version number (currently v1.4) at the top, which is incremented with each change for easy tracking. As of v1.1, peer comments that are just numbers (likely from adjacent columns) are filtered out and not shown. As of v1.3, median calculation matches the Python logic exactly.
